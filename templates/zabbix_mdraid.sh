...
do
	...
	"e")
	    ...
	    /usr/bin/mdadm --detail ${MD_dev} | grep "${OPTARG}" | awk -F":" '{print $2}' | tr -d [[:space:]]
            ;;
	"s")
            # echo "Size of the array"
	    /usr/bin/mdadm --detail ${MD_dev} | grep "${OPTARG}" | awk -F":" '{print $2}' | sed -e "s/(.*//" | tr -d [[:space:]]
            ;;
	"d")
            # echo "Devices in the array"
	    /usr/bin/mdadm --detail ${MD_dev} | tail -n+2 | grep "/dev/" | awk -v x=${OPTARG} '$4 == x {print $5,$6,$7}'
            ;;
	"m")
